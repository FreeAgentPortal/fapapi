...
export default class ReferencesService extends CRUDService {
  ...
  public create = asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<Response> => {
    try {
      ...
      const result = await this.handler.addReference(ownerKind, ownerRef, req.body);
  ...
  public updateResource = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const result = await this.handler.updateReference(ownerKind, ownerRef, id, patch);
  ...
  public deleteResource = asyncHandler(async (req: AuthenticatedRequest, res: Response): Promise<Response> => {
    ...
    try {
      const result = await this.handler.removeReference(id, itemId);
      return res.status(200).json(result);
    } catch (err) {
      return error(err, req, res);
    }
